...
(defun draw-dice-svg (x y col)
  (labels ((calc-pt (pt)
                    (cons (+ x (* *dice-scale* (car pt)))
                          (+ y (* *dice-scale* (cdr pt)))))
           (f (pol col)
              (polygon (mapcar #'calc-pt pol)
                       (style-fill-and-dark-stroke col))))
    ...
    (mapc (lambda (x y)
            (polygon (mapcar (lambda (xx yy)
                             ...
                             '(-1 -1 1 1)
                             '(-1 1 1 -1))
                     (style-fill-and-dark-stroke '(255 255 255))))
          '(-0.05 0.125 0.3 -0.3 -0.125 0.05 0.2 0.2 0.45 0.45 -0.45 -0.2)
          '(-0.875 -0.8 -0.725 -0.775 -0.7 -0.625 -0.35 -0.05 -0.45 -0.15 -0.45 -0.05))))
  ...
  (loop for z below 2
        do (polygon (mapcar (lambda (pt)
                    ...
                    (if (eql pos chosen-tile)
                      (style-fill-and-dark-stroke (brightness col 100))
                      (style-fill-and-dark-stroke col))))
  (loop for z below (second hex)
        do (draw-dice-svg (+ xx
                             (* *dice-scale* 0.3 (if (oddp (+ x y z))
                                                   -0.3
                                                   0.3)))
                          (- yy (* *dice-scale* z 0.8))
                          col)))
...
(defun dod-request-handler (path header params)
  ...
  (tag html () 
      (if (equal path "game.html")
        (progn (tag center ()
                    (draw-dod-web-ui params)))
        (princ "Sorry... i dont know that page."))))

(defun draw-dod-web-ui (params)
...
(defun web-handle-human (pos)
  (cond ((not pos) (princ "Please choose a hex to move from:"))
        ...
        (t (setf *cur-game-tree*
           ...
           (setf *from-tile* nil)
           (princ "You may now ")
           (tag a (href (make-game-link 'pass)) (princ "pass"))
